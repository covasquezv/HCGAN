...
check_dir('TRTR_'+ date)
check_dir('TRTR_'+ date +'/train/')
check_dir('TRTR_'+ date +'/train/')
check_dir('TRTR_'+ date +'/train/'+ folder)
check_dir('TRTR_'+ date +'/test/')
check_dir('TRTR_'+ date +'/test/'+ folder)


if os.path.isfile('TRTR_'+ date +'/train/'+ folder +'/train_model.h5'):

	print('\nTrain metrics:')

	mean = np.load('TRTR_'+ date +'/train/'+ folder +'/train_is_mean.npy')

	std = np.load('TRTR_'+ date +'/train/'+ folder +'/train_is_std.npy')

	print('Training metrics:')
	print('Inception Score:\nMean score : ', mean[-1])
	print('Std : ', std[-1])

	acc = np.load('TRTR_'+ date +'/train/'+ folder +'/train_history_acc.npy')
	val_acc = np.load('TRTR_'+ date +'/train/'+ folder +'/train_history_val_acc.npy')
	loss = np.load('TRTR_'+ date +'/train/'+ folder +'/train_history_loss.npy')
	val_loss = np.load('TRTR_'+ date +'/train/'+ folder +'/train_history_val_loss.npy')
	...
	score = np.load('TRTR_'+ date +'/train/'+ folder +'/test_score.npy')
	print('Test loss:', score[0])
	print('Test accuracy:', score[1])

	roc = np.load('TRTR_'+ date +'/train/'+ folder +'/test_rocauc.npy')
	print('auc roc', roc)
...
## Train on real
	...
	#if irr == True:
	#	X_train, y_train, X_val, y_val, X_test, y_test  = read_data_generated_irr('TRTR_data/generated/'+ folder +'/' + dataset_syn + '.pkl')
	#else:
	#	X_train, y_train, X_val, y_val, X_test, y_test  = read_data('/TRTR_data/generated/'+ folder + '/' + dataset_syn + '.pkl')

	# Load dataset
	dataset_real = 'catalina_random_full_north_9classes'
	if irr == True:
		X_train, y_train, X_val, y_val, X_test, y_test  = read_data_original_irr('TRTR_data/datasets_original/REAL/'+ dataset_real +'.pickle')
	else:
		X_train, y_train, X_val, y_val, X_test, y_test  = read_data('TRTR_data/datasets_original/REAL/'+ dataset_real +'.pkl')
...
## callbacks
	...
	checkpoint = ModelCheckpoint('TRTR_'+ date +'/train/'+ folder +'/weights.best.train.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='min')
	...
	model.save('TRTR_'+ date +'/train/'+ folder +'/train_model.h5')

	#Create dictionary, then save into two different documments.
## Loss
	history_dictionary_loss = history.loss
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_history_loss.npy', history_dictionary_loss)
## Val Loss
	history_dictionary_val_loss = history.val_loss
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_history_val_loss.npy', history_dictionary_val_loss)
## Acc
	history_dictionary_acc = history.acc
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_history_acc.npy', history_dictionary_acc)
## Val Acc
	history_dictionary_val_acc = history.val_acc
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_history_val_acc.npy', history_dictionary_val_acc)
## AUC ROC
	roc_auc_dictionary = rocauc.roc_auc
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_rocauc_dict.npy', roc_auc_dictionary)
## IS
	scores_dict = inception.score
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_is.npy', scores_dict)
	mean_scores_dict = inception.mean
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_is_mean.npy', mean_scores_dict)
	std_scores_dict = inception.std
	np.save('TRTR_'+ date +'/train/'+ folder +'/train_is_std.npy', std_scores_dict)
...
### plot loss and validation_loss v/s epochs
	...
	plt.savefig('TRTR_'+ date +'/train/'+ folder +'/train_loss.png')
### plot acc and validation acc v/s epochs
	...
	plt.xlabel('epoch')
	plt.legend(['train', 'val'], loc='upper right')
	plt.savefig('TRTR_'+ date +'/train/'+ folder +'/train_acc.png')
	...
	np.save('TRTR_'+ date +'/test/'+ folder +'/test_is.npy', sc)
	np.save('TRTR_'+ date +'/test/'+ folder +'/test_is_mean.npy', me)
	np.save('TRTR_'+ date +'/test/'+ folder +'/test_is_std.npy', st)

	score = model.evaluate(X_test, y_test, verbose=1)
	print('Test loss:', score[0])
	print('Test accuracy:', score[1])

	np.save('TRTR_'+ date +'/test/'+ folder +'/test_score.npy', score)

	y_pred = model.predict(X_test)
	roc = roc_auc_score(y_test, y_pred)
	print('auc roc', roc)
	np.save('TRTR_'+ date +'/test/'+ folder +'/test_rocauc.npy', roc)